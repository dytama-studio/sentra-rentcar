...
export type SignUpFormRefType = {
  ...
  setValue: UseFormSetValue<UserSignUp>;
};

const SignUpModule: ForwardRefRenderFunction<SignUpFormRefType, Props> = (
        ...
        <div className="relative w-full lg:max-w-sm">
          <div className="text-center space-y-3">
            <div className="flex flex-col space-y-2 pt-5">
              <h4 className="text-2xl lg:text-3xl text-black font-bold">
                Register Akun Dytama
              </h4>
              <p className="text-gray-500 text-xs lg:text-sm font-normal">
                Silahkan registrasikan terlebih dahulu
              </p>
            </div>
          </div>
          {errorMsg && (
          ...
          <form className="py-5 lg:py-5" onSubmit={handleSubmit(onSubmit)}>
            <div className="grid gap-y-4">
              ...
              <button
                type="submit"
                className="w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-primary text-white hover:bg-indigo-800 disabled:opacity-50 disabled:pointer-events-none"
                disabled={onLoading}
              >
                {onLoading ? (
                  <>
...
export default forwardRef(SignUpModule);
