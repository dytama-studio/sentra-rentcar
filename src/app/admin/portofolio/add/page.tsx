...
const AddPortofolio = () => {
  ...
  const { mutate: postPortofolio } = api.portofolios.store.useMutation();
  ...
  const onSubmit: SubmitHandler<PortofolioValues> = async (values: any) => {
    ...
    try {
      ...
      const payload = { ...values, cover: fileUrl };
      postPortofolio(payload, {
        onSuccess: () => {
          toast.success("Portofolio has been created");
